...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPerformance = async () => {
      try {
        ...
        setProgress(data.performance_percentage);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  const handlePlanUpdate = (plan: any, type: "workout" | "diet") => {
    let total = 0;
    let completed = 0;
    if (type === "workout" && plan.days) {
      ...
      completed = plan.meals.reduce((sum: number, day: any) => {
        let dayCompleted = 0;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <aside className="lg:col-span-1 space-y-8">
              <DashboardSection title="Statistics and Score">
                <GamificationCard performance={progress} />
              </DashboardSection>
            </aside>
          </div>
        </main>
